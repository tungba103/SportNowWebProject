...
import { getAllProduct, getProductSearch, getProductSearchByPriceRange, getProductSearchSorted } from '../../components/API/Product';
...
function SearchPage(props) {
  ...
  useEffect(() => {
    if (typeof search.search === 'undefined') {
      getAllProduct((data) => setSearchProducts(data));
    } else getProductSearch((data) => setSearchProducts(data), search.search);
  }, [search.search]);
      ...
      <div>
        ...
        <div id="item-list" className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-8">
          {typeof searchProducts === 'undefined' ? (
            ...
            <div>
              <h1 className="font-bold p-2">There is no product available!</h1>
              <Button>
